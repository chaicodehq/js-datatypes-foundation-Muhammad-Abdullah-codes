...
export function makeRangoliBorder(char, length) {
  ...
  let repChar = char.repeat(100);
  let sliceChar = repChar.slice(0, length);
  return sliceChar;
}
...
export function splitAndJoinRangoli(colorString, oldSep, newSep) {
  if (typeof colorString !== "string") {
    ...
    let breaked = colorString.split(oldSep);
    let joined = breaked.join(newSep);
    return joined;
  }
}

export function extractRangoliCenter(design, start, end) {
  if (typeof design !== "string" || start < 0 || end >= design.length) {
    return "";
  } else {
